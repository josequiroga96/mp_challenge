...
class ItemMapperTest {
    ...
    @Test
    void toPartialEntity_ShouldCreateEntityWithOnlyProvidedFields_WhenUpdateDto() {
        // Given
        UpdateItemDto updateDto = UpdateItemDto.builder()
                .id(UUID.randomUUID())
                .name("Partial Name")
        ...
        // When
        Item result = ItemMapper.toPartialEntity(updateDto);

        // Then
        assertNotNull(result);
        assertEquals(updateDto.getId(), result.getId());
        assertEquals("Partial Name", result.getName());
        assertNull(result.getImageUrl()); // Not provided
        assertNull(result.getDescription()); // Not provided
        assertEquals(new BigDecimal("50.00"), result.getPrice());
        assertNull(result.getRating()); // Not provided
        assertNull(result.getSpecifications()); // Not provided
        assertNull(result.getCreatedAt()); // Not available in update DTO
        assertNull(result.getUpdatedAt()); // Not available in update DTO
    }

    @Test
    void toPartialEntity_ShouldThrowException_WhenUpdateDtoIsNull() {
        // When & Then
        ValidationException exception = assertThrows(
                ValidationException.class,
                () -> ItemMapper.toPartialEntity(null)
        );
        assertEquals("Validation failed for field 'updateItemDto': UpdateItemDto cannot be null", exception.getMessage());
        assertEquals("VALIDATION_FAILED", exception.getErrorCode());
    }
    ...
    @Test
    void touchEntity_ShouldUpdateTimestamp_WhenValidItem() {
        // Given
        Item item = createTestItem();
        ...
        // When
        Item result = ItemMapper.touchEntity(item);
    ...
    @Test
    void touchEntity_ShouldThrowException_WhenItemIsNull() {
        // When & Then
        ValidationException exception = assertThrows(
                ValidationException.class,
                () -> ItemMapper.touchEntity(null)
        );
        assertEquals("Validation failed for field 'item': Item cannot be null", exception.getMessage());
        assertEquals("VALIDATION_FAILED", exception.getErrorCode());
    }
